...
{
  const auto output =
    gyro_odometer_.input_vehicle_twist(*vehicle_twist_msg_ptr, this->now());
  ...
  const AUTOWARE_MESSAGE_CONST_SHARED_PTR(sensor_msgs::msg::Imu) imu_msg_ptr)
{
  ...
  if (!transformed_imu_msg) {
    gyro_odometer_.discard_unusable_imu(*imu_msg_ptr, this->now());
    return;
  }

  const auto output =
    gyro_odometer_.input_imu(*transformed_imu_msg, this->now());
