...
protected:
  void SetUp() override
  {
    ...
    driver_node_ = std::make_shared<rclcpp::Node>("gyro_odometer_test_driver");
...
// Scenarios that need a fusion to happen send a vehicle twist, an IMU sample and a vehicle twist
// again, rather than one of each. The first message of either kind only marks its side as arrived
// and is then dropped, so a pair has to be built up before anything can come out.
// TODO(youtalk): Send one of each after the refinement of the GyroOdometerNode

// The four output topics each carry their own half of the fusion. Values that put the vehicle at a
// standstill are used because they are the only ones that tell the raw pair and the compensated
// pair apart, which is what makes this a check of where each message is published rather than of
// what the fusion computed.
TEST_F(GyroOdometerNodeTest, EachOutputTopicCarriesItsOwnHalfOfTheFusion)
{
  start_node("base_link", 10.0);
  const auto stamp = make_stamp(100, 0);

  send_vehicle_twist(make_vehicle_twist(stamp, 0.0, 0.0));
  send_imu(make_imu(stamp, "base_link", 0.5, 0.6, 0.0, 0.01, 0.02, 0.03));
  send_vehicle_twist(make_vehicle_twist(stamp, 0.0, 4.0));

  const auto output = take_output();
  ASSERT_TRUE(output.has_value()) << "not all four topics were published to";
  ...
  EXPECT_EQ(output->twist_with_covariance_raw.header.frame_id, "base_link");
}
...
// An IMU frame that cannot be resolved into the output frame keeps the sample out of the fusion
// and says so through the diagnostics.
...
// With the IMU frame resolvable, the angular velocity is rotated by the looked-up transform before
// it is fused, and the sample is relabelled as belonging to the output frame.
TEST_F(GyroOdometerNodeTest, ResolvableImuFrameRotatesTheAngularVelocity)
{
  ...
  // The angular variance is reduced to the largest of the reported axes, applied to all three.
...
TEST_F(GyroOdometerNodeTest, StaleAndUnresolvableImuIsReportedAsBoth)
{
...
// IMU samples with no vehicle twist to pair them with publish nothing, and the transform status
// still describes the sample that arrived most recently.
TEST_F(GyroOdometerNodeTest, ImuAlonePublishesNothingAndStillReportsItsTransformStatus)
{
  start_node("base_link", 10.0);
  const auto stamp = make_stamp(100, 0);
